...
class IngredientPicker extends React.Component {
  ...
  handleSubmit(event) {
    ...
    fetch("https://api.nal.usda.gov/fdc/v1/foods/search?api_key=DEMO_KEY&query=" + encodeURIComponent(this.state.value))
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
  ...
  handleClick = value => () => {
    ...
    fetch("https://api.nal.usda.gov/fdc/v1/food/" + encodeURIComponent(value) + "?api_key=DEMO_KEY")
      .then(res => res.json())
      .then(
        (result) => {
          this.setState({
